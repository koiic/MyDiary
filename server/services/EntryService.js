...
class EntryService {
   ...
   static addEntry(data) {
    ...
    static findById(id) {
        const error = DummyDataHelpers.validateId(id);
        if(!error){
    ...
    static todaysEntryVolume() {
        ...
        return this.success("Today's entry count fetched successfully", todayEntriesCount);
    }
    ...
    static favouriteEntries(){
        ...
        if (favouriteEntries === undefined || favouriteEntries.length == 0) {
